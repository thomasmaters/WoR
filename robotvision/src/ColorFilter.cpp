...
cv::Mat ColorFilter::applyFilter(const cv::Mat& source, const ColorFilter::Color& color, bool showResult)
{
    ...
    if ((color & ColorFilter::Color::BLACK) == ColorFilter::Color::BLACK)
    {
        temp_source = filter.applyFilter(
            source, ImageFilter::FilterType::MEDIANBLUR | ImageFilter::FilterType::PYRMEANSHIFT, showResult);
        applyRange(temp_source, cv::Scalar(0, 0, 0), cv::Scalar(180, 255, 15), result);
    }
    if ((color & ColorFilter::Color::BLUE) == ColorFilter::Color::BLUE)
    {
        temp_source = filter.applyFilter(
            source, ImageFilter::FilterType::GAUSSIANBLUR | ImageFilter::FilterType::PYRMEANSHIFT, showResult);
    ...
    if ((color & ColorFilter::Color::RED) == ColorFilter::Color::RED)
    {
        ...
        temp_source = filter.applyFilter(invertedSource,
                                         ImageFilter::FilterType::GAUSSIANBLUR | ImageFilter::FilterType::EQUALIZEHIST |
                                             ImageFilter::FilterType::PYRMEANSHIFT,
                                         showResult);
...
void ColorFilter::applyRange(const cv::Mat& source, const cv::Scalar& lowerBound, const cv::Scalar& upperBound,
                             cv::Mat& result)
{
