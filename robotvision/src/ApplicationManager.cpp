...
ApplicationManager::ApplicationManager()
  ...
  , colorFinder(ColorFilter())
  , shapeFinder(ShapeFilter())
  ...
  , showFilterProgress(false)
{
    ...
    //shapeFinder.setRealLifeConversionRate(colorFinder.applyFilter(inputHandler.getVideoCaptureFrame(),ColorFilter::Color::WHITE), 100);
}
...
void ApplicationManager::specialCommandParser(std::string& command)
{
    ...
    {
    	shapeFinder.setRealLifeConversionRate(colorFinder.applyFilter(inputHandler.getWebcamFrame(),ColorFilter::Color::WHITE, showFilterProgress), std::stoi(match[1]), showFilterProgress);
        return;
    }
...
{
    cv::Mat frame = inputHandler.getWebcamFrame();
    if (!frame.empty())
    {
